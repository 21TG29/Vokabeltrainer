...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const container = document.getElementById('lektionenListe');
  container.innerHTML = '';
  const gruppen = Object.keys(tabellen).sort();
  gruppen.forEach(gruppeName => {
    const lektionen = Object.keys(tabellen[gruppeName] || {}).sort();
    const details = document.createElement('details');
    details.className = 'gruppe';
    if (offeneGruppen.has(gruppeName)) details.open = true;
    const summary = document.createElement('summary');
    summary.className = 'gruppeSummary';
    const header = document.createElement('div');
    header.className = 'gruppeHeader';
    const links = document.createElement('div');
    links.className = 'gruppeLinks';
    const toggleIcon = document.createElement('div');
    toggleIcon.className = 'toggleIcon';
    toggleIcon.textContent = details.open ? '−' : '+';
    const titelWrap = document.createElement('div');
    const titel = document.createElement('div');
    titel.className = 'gruppeTitel';
    titel.textContent = gruppeName;
    const info = document.createElement('div');
    info.className = 'gruppeInfo';
    info.textContent = `${lektionen.length} Lektionen`;
    titelWrap.appendChild(titel);
    titelWrap.appendChild(info);
    links.appendChild(toggleIcon);
    links.appendChild(titelWrap);
    const gruppeBtn = document.createElement('button');
    gruppeBtn.type = 'button';
    gruppeBtn.className = 'gruppeAuswahlBtn';
    gruppeBtn.setAttribute('aria-expanded', details.open ? 'true' : 'false');
    const keys = lektionen.map(l => `${gruppeName}__${l}`);
    const selectedCount = keys.filter(k => ausgewaehlt.has(k)).length;
    ...
    const unterWrap = document.createElement('div');
    unterWrap.className = 'untermenge';
    lektionen.forEach(lektionName => {
      const key = `${gruppeName}__${lektionName}`;
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'lektionBtn';
